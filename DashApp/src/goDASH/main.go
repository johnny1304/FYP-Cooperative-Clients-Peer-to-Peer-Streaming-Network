...
// Cooperative Code - Start
//file download location == argument --serveraddr
var fileDownloadLocation = os.Args[4]
...
func main() {
	...
	// this sets each flag
	serverAddrPtr := flag.String(glob.ServerAddr, "", "Specify file location of downloaded content. For use with Evaluate function")
	...
	fmt.Println("server addr: " + os.Args[4])
	s := strings.Split(os.Args[4], "/")
	var wg = &sync.WaitGroup{}
	//Create Node instance
	Noden := P2Pconsul.NodeUrl{}

	//Set node for no-collaborative state
	if os.Args[4] == "off" {
		Noden = P2Pconsul.NodeUrl{
			ClientName: "off",
		}
		//set reference to Node in http class
		http.SetNoden(Noden)
	} else {
		//Set node for collaborative state
		Noden = P2Pconsul.NodeUrl{
			//Client name for correct content location
			ClientName: s[len(s)-1],
			//Content location to be server by http server
			ContentLocation: os.Args[4],
			//initiralise clients
			Clients: nil,
			//Consul address for goDASHbed
			//SDAddress:       "10.0.0.2:8500",
			//Consul address for evaluate function and standard running
		...
		//set http reference of node
		http.SetNoden(Noden)

		//add wait group to keep clients from terminating prematurely
		wg.Add(1)

		//go routine starting GRPC server of Node class
		go Noden.StartListening(wg)
		wg.Add(1)
		//go routine starting http server of node class
	...
	if utils.IsFlagSet(glob.ServerAddr) {
		fmt.Println("Server flag" + *serverAddrPtr)
	}

	// Collaborative Code - Start
	//creates correct file downlaod location for case when collaborative clients is turned off
	if fileDownloadLocation == "off" {
		fileDownloadLocation = filepath.Join("../files/", *fileStorePtr)
	}
	...
	player.Stream(structList, glob.DebugFile, debugLog, *codecPtr, glob.CodecName, *maxHeightPtr,
		*streamDurationPtr, *maxBufferPtr, *initBufferPtr, *adaptPtr, *urlPtr, fileDownloadLocation, extendPrintLog, *hlsPtr, hlsBool, *quicPtr, quicBool, getHeaderBool, *getHeaderPtr, exponentialRatio, printHeadersData, printLog, useTestbedBool, true, Noden)
