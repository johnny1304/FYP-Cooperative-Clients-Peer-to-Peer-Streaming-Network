...
func Stream(mpdList []http.MPD, debugFile string, debugLog bool, codec string, codecName string,
	...
	useTestbedBoolIn bool, getQoEBoolIn bool, Noden P2Pconsul.NodeUrl) {
	//set debug to true if in collaborative mode
	...
	logging.DebugPrint(debugFile, debugLog, "DEBUG: ", fmt.Sprintf("file download location ** : %v\n", fileDownloadLocation))
	...
	//in collaborative mode search local network for url content
	if Noden.ClientName != "off" {
	...
	segmentNumber, mapSegmentLogPrintout = streamLoop(segmentNumber, OriginalURL, initBuffer, maxBuffer, codecName, codec, urlString, urlInput,
		...
		extendPrintLog, false, bufferLevel, segmentDurationTotal, quic, quicBool, OriginalbaseURL, debugLog, Noden)
	// Collaborative Code - End
