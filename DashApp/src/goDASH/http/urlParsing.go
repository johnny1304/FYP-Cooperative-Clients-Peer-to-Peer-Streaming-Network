...
import (
	...
	"fmt"
	"goDASH/P2Pconsul"
	"goDASH/P2Pconsul/HelperFunctions"
	"goDASH/logging"
	"goDASH/utils"
	"path/filepath"
...
func getURLBody(url string, isByteRangeMPD bool, startRange int, endRange int, quicBool bool, debugFile string, debugLog bool, useTestbedBool bool) (io.ReadCloser, time.Duration, string) {
	...
	if useTestbedBool {
		...
		if err != nil {
			...
			logging.DebugPrint(debugFile, debugLog, "DEBUG: ", "unable to read X509 cert: "+dir+"/"+glob.HTTPcertLocation+" "+dir+"/"+glob.HTTPkeyLocation)
			log.Fatal(err)
		}
	...
	if quicBool {
		...
		if useTestbedBool {
			...
			// set up our http transport
			logging.DebugPrint(debugFile, debugLog, "DEBUG: ", "creating our http transppppport using our tls config")
	...
	if err != nil {
		...
		logging.DebugPrint(debugFile, debugLog, "DEBUG: ", "URL doesnt match")
		// stop the app
		utils.StopApp()
	}
	...
	if quicBool {
		...
		recuriveTestbedCall = func(count int) {
			...
			if count > 5 {
				...
				logging.DebugPrint(debugFile, debugLog, "DEBUG: ", "unable to connect to the URL on the testbed")
				fmt.Println(err)
				// stop the app
				os.Exit(3)
			} else if err != nil {
		...
		recuriveDefaultCall = func(count int) {
			...
			if count > 5 {
				...
				logging.DebugPrint(debugFile, debugLog, "DEBUG: ", "unable to connect to url using default settings")
				fmt.Println(err)
				// stop the app
				os.Exit(3)
			} else if err != nil {
...
func GetURL(url string, isByteRangeMPD bool, startRange int, endRange int, quicBool bool, debugFile string, debugLog bool, useTestbedBool bool) ([]byte, time.Duration, string) {
	...
	if err != nil {
		...
		logging.DebugPrint(debugFile, debugLog, "DEBUG: ", "unable to read from url")
		// stop the app
		utils.StopApp()
	}
...
func JoinURL(baseURL string, append string, debugLog bool) string {
	...
	if !(strings.Contains(append, "http")) {
		...
		// replace this base url with the required fihttps://www.godashbed.org/ed_10min/DASH_Files/VOD/ed_enc_10min_x264_dash.mpd]le string
...
func GetFile(currentURL string, fileBaseURL string, fileLocation string, isByteRangeMPD bool, startRange int, endRange int,
	...
	logging.DebugPrint(debugFile, debugLog, "DEBUG: ", "get file from filebase prior: "+fileBaseURL+"\n")
	var filesplit = strings.Split(fileBaseURL, "/")
	urlHeaderString := JoinURL(currentURL, filesplit[len(filesplit)-1], debugLog)
	fmt.Printf("URLHEADER STRING: %v\n", urlHeaderString)
	logging.DebugPrint(debugFile, debugLog, "DEBUG: ", "get file from currentURL: "+currentURL+"\n")

	logging.DebugPrint(debugFile, debugLog, "DEBUG: ", "get file from filebase: "+fileBaseURL+"\n")
	...
	if Noden.ClientName == "off" {
		...
		logging.DebugPrint(debugFile, debugLog, "DEBUG: ", "(off) get file from URL: "+urlHeaderString+"\n")
	...
	if err != nil {
		...
		logging.DebugPrint(debugFile, debugLog, "DEBUG: ", "*** "+createFile+" cannot be downloaded ***")
		// stop the app
		utils.StopApp()
	}
	defer out.Close()
	...
	if err != nil {
		fmt.Printf("*** "+createFile+" cannot be saved *** %v\n", err)
		logging.DebugPrint(debugFile, debugLog, "DEBUG: ", "*** "+createFile+" cannot be saved ***"+err.Error())
	...
	if err != nil {
		fmt.Println(err)
		logging.DebugPrint(debugFile, debugLog, "DEBUG: ", fmt.Sprintf("create file : %v\n", err))
...
func GetFileProgressively(currentURL string, fileBaseURL string, fileLocation string, isByteRangeMPD bool, startRange int, endRange int, segmentNumber int, segmentDuration int, addSegDuration bool, debugLog bool) (time.Duration, int) {
	...
	// join the new file location to the base url
	urlHeaderString := JoinURL(currentURL, fileBaseURL, debugLog)
	logging.DebugPrint(glob.DebugFile, debugLog, "DEBUG: ", "get file progrssily from URL: "+urlHeaderString+"\n")
