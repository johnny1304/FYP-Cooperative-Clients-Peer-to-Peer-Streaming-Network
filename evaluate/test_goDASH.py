...
parser.add_argument('--cooperative',
                    dest="cooperative",
                    help="run network in cooperative mode",
                    default="off")
...
def eval_goDASH():
    ...
    for i in range(1, int(args.numClients)+1):
       ...
       # cmd = cwd+"/../DashApp/src/goDASH/goDASH --config " + \
        #    output_folder+current_folder+config_folder_name+client_config +\
         #   " --serveraddr ../../../../evaluate/output"+current_folder+"/files/"+client_name
    ...
    if getHeaders:
        ...
        for i in range(1, int(args.numClients)+1):
            ...
            # lets call goDASH and get some output
            if args.cooperative == "on":
                cmd = cwd+"/../DashApp/src/goDASH/goDASH --config " + \
                    output_folder+current_folder+config_folder_name+client_config+\
                    " --serveraddr ../../../../evaluate/output"+current_folder+"/files/"+client_name
            elif args.cooperative == "off":
                cmd = cwd+"/../DashApp/src/goDASH/goDASH --config " + \
                      output_folder+current_folder+config_folder_name+client_config+ \
                      " --serveraddr off"
